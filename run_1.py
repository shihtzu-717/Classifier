                        ...
                        for ncls in nb_classes:
                            for st in soft_type: 
                                for lf in loss_func:
                                    ...
                                    name = f'pad_{pad}_padsize_{pad_size:.1f}_box_{bbox}_shift_{shift}_sratio_{soft_ratio}_tratio_{target_ratio}_nbclss_{ncls}_tmp'
                                    output_dir_name = org_output_dir_name
                                    if ncls == 4:
                                        name += f'_soft-type_{st}_loss-func_{lf}'
                                        output_dir_name += f"/4-class-soft_type-{st}"
                                    if ncls == 2:
                                        name += f'_loss-func_{lf}'
                                    ...
                                    if not os.path.isdir(os.getcwd() + '/log/' + name):
                                        ...
                                        print("Check the", os.getcwd() + '/log/' + name)
